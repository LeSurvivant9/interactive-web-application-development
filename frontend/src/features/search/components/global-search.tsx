...
import { type ISearchQuery, SearchDocument } from "@/graphql/generated";
import { useDebounce } from "@/hooks/use-debounce";
import { cn } from "@/lib/utils";

type SearchResultItem = ISearchQuery["searchMedia"][number];
...
function SearchItem({ media, onSelect }: SearchItemProps) {
  return (
    <CommandItem
      value={`${media.name} ${media.tvdb_id}`}
      onSelect={() => onSelect(media.tvdb_id || "", media.type || "unknown")}
...
function SearchResults({
  ...
  if (data?.searchMedia?.length === 0 && debouncedQuery.length >= 3) {
    return <CommandEmpty>Aucun résultat trouvé.</CommandEmpty>;
  }

  if (data?.searchMedia && data.searchMedia.length > 0) {
    return (
      <CommandGroup heading="Résultats">
        {data.searchMedia.map((media: SearchResultItem) => (
          <SearchItem key={media.tvdb_id} media={media} onSelect={onSelect} />
        ))}
      </CommandGroup>
    );
  }
